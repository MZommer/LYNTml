...
for file in os.listdir(input_dir):
    ...
    if input_file.is_file():
        ...
        for index, move in enumerate(bluestar.moves):
            if move:
                with open(
                    song_dir / f"{timeline.general.Song}_Moves{index}.json", "w"
                ) as f:
                    json.dump(move, f)
        for index, move in enumerate(bluestar.kinectmoves):
            if move:
                with open(
                    song_dir / f"{timeline.general.Song}_KinectMoves{index}.json", "w"
                ) as f:
                    json.dump(move, f)
        ...
        for _gesture in timeline.databank.GestureBank:
            pass  # TODO: add generic gesture

        if BlueStar:
            os.makedirs(song_dir / "UAF", exist_ok=True)
            song = BlueStar.Song(
                **bluestar.main, moves=bluestar.moves, kinectMoves=bluestar.kinectmoves
            )
            song.makeUAF()
