...
from typing import Sequence, List, Iterable, Union, Optional
...
class PictoBank(ET.Element):
    ...
    def add_picto(self, name: str, CreationId: int, offset: float = None) -> Picto:
...
class Move(ET.Element):
    ...
    def __init__(self,
                 name: str, CreationId: int, duration: int, SubdivisionsInBeat: int, color: str,
                 livemovemul: float, livemoveplus: float, Slack: float, Capacity: float, Stability: float,
                 GoldenMove: bool, EnergyEvaluation: bool, TimingEvaluation: bool,
                 CustomFloats: Sequence[float]
                 ) -> None:
        super().__init__("Move")
        self.name = name
    ...
    @property
    def CustomFloats(self) -> List[float]:
        return [float(i) for i in self.CustomFloatsElement.text.split(';')]

    @CustomFloats.setter
    def CustomFloats(self, value: Union[str, Iterable[float]]) -> None:
        if isinstance(value, str):
            self.CustomFloatsElement.text = value
        else:
            self.CustomFloatsElement.text = ';'.join(str(i) for i in value)
...
class MoveBank(ET.Element):
    ...
    def add_move(self, name: str, CreationId: int, duration: int, SubdivisionsInBeat: int, color: str,
                 livemovemul: float, livemoveplus: float, Slack: float, Capacity: float, Stability: float,
                 GoldenMove: bool, EnergyEvaluation: bool, TimingEvaluation: bool,
                 CustomFloats: Sequence[float]) -> Move:
        move = Move(name, CreationId, duration, SubdivisionsInBeat,
                    color, livemovemul, livemoveplus, Slack, Capacity,
                    Stability, GoldenMove, EnergyEvaluation, TimingEvaluation, CustomFloats)
        self.append(move)
        return move
...
class Event(ET.Element):
    ...
    def add_param(self, name: str, _type: str = '', DisplayInTimeline: int = 1, DefaultValue: str = '') -> Param:
        param = Param(name, _type, DisplayInTimeline, DefaultValue)
...
class EventsBank(ET.Element):
    ...
    def add_event(self, name: str, CreationId: int, SubDivisionsInBeat: int, DefaultDuration: int) -> Event:
...
class Gesture(ET.Element):
    ...
    def __init__(self, name: str, CreationId: int, duration: int, SubdivisionsInBeat: int, color: str,
                 gesturemul: float, gestureplus: float, Slack: float, Capacity: float, Stability: float,
                 GoldenMove: bool, EnergyEvaluation: bool, TimingEvaluation: bool,
                 CustomFloats: Sequence[float]) -> None:
        super().__init__("Move")
        self.name = name
        self.CreationId = CreationId
...
class GestureBank(ET.Element):
    ...
    def add_gesture(self, name: str, CreationId: int, duration: int, SubdivisionsInBeat: int, color: str,
                    gesturemul: float, gestureplus: float, Slack: float, Capacity: float, Stability: float,
                    GoldenMove: bool, EnergyEvaluation: bool, TimingEvaluation: bool,
                    CustomFloats: Sequence[float]) -> Gesture:
        gesture = Gesture(name, CreationId, duration, SubdivisionsInBeat, color, gesturemul, gestureplus, Slack,
                          Capacity, Stability, GoldenMove, EnergyEvaluation, TimingEvaluation, CustomFloats)
        self.append(gesture)
        return gesture
...
class DataBank(ET.Element):
    ...
    def add_picto(self, name: str, CreationId: int, offset: float = None) -> Picto:
        picto = self.PictoBank.add_picto(name, CreationId, offset)
    ...
    def add_move(self, name: str, CreationId: int, duration: int, SubdivisionsInBeat: int, color: str,
                 ...
                 GoldenMove: bool, EnergyEvaluation: bool, TimingEvaluation: bool,
                 CustomFloats: Sequence[float]) -> Move:
        move = self.MoveBank.add_move(name, CreationId, duration, SubdivisionsInBeat, color, livemovemul, livemoveplus,
                                      Slack, Capacity, Stability, GoldenMove, EnergyEvaluation, TimingEvaluation,
                                      CustomFloats)
        self.MainBank.append(move)
        return move

    def add_event(self, name: str, CreationId: int, SubDivisionsInBeat: int, DefaultDuration: int) -> Event:
        event = self.EventsBank.add_event(name, CreationId, SubDivisionsInBeat, DefaultDuration)
    ...
    def add_gesture(self, name: str, CreationId: int, duration: int, SubdivisionsInBeat: int, color: str,
                    ...
                    GoldenMove: bool, EnergyEvaluation: bool, TimingEvaluation: bool,
                    CustomFloats: Sequence[float]) -> Gesture:
        gesture = self.GestureBank.add_gesture(name, CreationId, duration, SubdivisionsInBeat, color, gesturemul,
                                               gestureplus, Slack, Capacity, Stability, GoldenMove, EnergyEvaluation,
                                               TimingEvaluation, CustomFloats)
        self.MainBank.append(gesture)
        return gesture

    def find_bank(self, name: str) -> Optional[ET.Element]:
